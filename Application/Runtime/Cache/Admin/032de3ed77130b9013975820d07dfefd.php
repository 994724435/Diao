...
<div class="rightfix">
  <a class="home1"></a><a class="home_pass">友情链接</a><a class="home_now">文字连接</a><a href="/Admin/Link/add" class="home_right">添加文字链接</a>
  <div class="clear"></div>
  <div class="article_list">
    <div class='check_page'>
      <span id="check_title">选择页面</span>
      <select name="check_page" id="check_">  
        <?php if(is_array($words_page)): foreach($words_page as $key=>$val): ?><option value="<?php echo ($val["bp_id"]); ?>" <?php if($bp_id == $val['bp_id']){echo "selected='selected'";} ?>><?php echo ($val["bp_name"]); ?></option><?php endforeach; endif; ?>
      </select>
    </div>
    <div class="clear"></div>
    ...
    <div class="clear"></div>
    <?php if(is_array($w_link)): foreach($w_link as $key=>$val): ?><div class="table_list">
        <div class="wz_list">
          <div class="wz_table1"><input type="checkbox" class="zi_choose"/></div>
          <div class="wz_table4"><input type="text" value="<?php echo ($val["wl_name"]); ?>" wlid="<?php echo ($val["wl_id"]); ?>" class="wl_name"></div>
          <div class="wz_table5"><input type="text" value="<?php echo ($val["wl_gourl"]); ?>" wlid="<?php echo ($val["wl_id"]); ?>" class="wl_gourl"></div>
          <div class="wz_table3"><input type="tel" value="<?php echo ($val["wl_order"]); ?>" wlid="<?php echo ($val["wl_id"]); ?>" class="wl_order" /></div>
          <div class="wz_table2"><a class="aa1" href="<?php echo U('/Admin/Link/words_del',array('wid'=>$val['wl_id']));?>">删除</a></div>
        </div>
      </div>
      <div class="clear"></div><?php endforeach; endif; ?>
    <div class="pagelist">
      <a class="delete">删除所选</a><a class="save">保存修改</a>
    </div>
  </div>
</div>
<script type="text/javascript">
  $('#check_').change(function(){
    var bp_id = $(this).val();
    var url = '/Admin/Link/words/bp/'+bp_id;
...
<script>
  ...
  var block_zIndex = 0;
  var ms_zIndex = -1;
  var blockName = ['changePassword_block','addAdmin_block','changePermissions_block','friendShip2_notice_block','setPermissions_block'];
  ...
  for(var i = 0;i < blockName.length;i++){
    if($(this).hasClass(blockName[i])){
      $('.'+blockName[i]).fadeIn(188);
      $('.'+blockName[i]).css('zIndex',block_zIndex);
    }
  }
