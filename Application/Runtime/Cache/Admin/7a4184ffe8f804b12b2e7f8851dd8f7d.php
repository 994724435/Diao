...
<div class="rightfix">
    <a href="<?php echo U('/Admin/Index/index');?>" class="home1"></a><a class="home_pass">文章管理</a><a class="home_now">文章列表</a>
    <div class="clear"></div>
    <div class="article_list" style="width:980px;">
        ...
        <div class="title">
          ...
          <div id="dd" class="wrapper-dropdown-3" tabindex="1">
            ...
            <ul class="dropdown"> 
              <li><a onclick="javascript:window.location.href='<?php echo U('/Admin/Article/index');?>'">未分类</a></li> 
              <?php if(is_array($acate)): foreach($acate as $key=>$value): ?><li><a onclick="javascript:window.location.href='<?php echo U('/Admin/Article/index',array('cid'=>$value['cate_id']));?>'"><?php echo ($value["cate_name"]); ?></a></li><?php endforeach; endif; ?> 
            </ul> 
          </div> 
          <div id="aa" class="wrapper-dropdown-3" tabindex="1"> 
            ...
            <?php if($cid != 0): ?><ul class="dropdown">  
                <li><a onclick="javascript:window.location.href='<?php echo U('/Admin/Article/index',array('cid'=>$cate['cate_id']));?>'">未分类</a></li> 
                <?php if(is_array($scate)): foreach($scate as $key=>$val): ?><li><a onclick="javascript:window.location.href='<?php echo U('/Admin/Article/index',array('cid'=>$cate['cate_id'],'sid'=>$val['cate_id']));?>'"><?php echo ($val["cate_name"]); ?></a></li><?php endforeach; endif; ?> 
              </ul> 
            <?php else: ?> 
              <ul class="dropdown"> 
                <li><a onclick="javascript:window.location.href='<?php echo U('/Admin/Article/index',array('cid'=>$cate['cate_id']));?>'">未分类</a></li> 
          ...
          <form class="allen" action="<?php echo U('/Admin/Article/index');?>"> 
        ...
        <div class="clear"></div>
        <!-- 文章列表  -->
        <form action="<?php echo U('Admin/Article/formDelete');?>" style="margin:20px 20px;" method="post">
          <table> 
            ...
            <?php if(is_array($news)): foreach($news as $key=>$value): ?><tr class="table_list"> 
                <td class="table1"><input type="checkbox" value="<?php echo ($value['art_id']); ?>" name="idarr[]"/></td> 
                <td class="table4"><?php echo ($value["art_title"]); ?></td> 
                <td div class="table2"> 
                  <?php if($value['art_isshow'] == 1): ?><a class="aa1" href="javascript:;" isshow="1" artID="<?php echo ($value["art_id"]); ?>" style="display:inline">隐藏</a>
                    <a href="javascript:;" class="aa4" isshow="0" artID="<?php echo ($value["art_id"]); ?>" style="display:none" >显示</a> 
                  <?php else: ?>
                    <a href="javascript:;" class="aa1" isshow="1" artID="<?php echo ($value["art_id"]); ?>" style="display:none">隐藏</a>
                    <a href="javascript:;" class="aa4" isshow="0" artID="<?php echo ($value["art_id"]); ?>" style="display:inline" >显示</a><?php endif; ?> /  <a href="<?php echo U('/Admin/Article/editor',array('aid'=>$value['art_id']));?>" class="aa2">编辑</a> /  <a href="<?php echo U('/Admin/Article/delete',array('aid'=>$value['art_id']));?>" class="aa3">删除</a>/<a class="pinglun" href="<?php echo U('/Admin/Article/comment',array('aid'=>$value['art_id']));?>"><?php if($value['mentNum']){echo "评论(".$value['mentNum'].")";}else{echo "评论(0)";} ?></a>
                </td> 
                <td class="table3"><input type="text" value="<?php echo ($value["art_order"]); ?>" artID="<?php echo ($value["art_id"]); ?>" class="order"/></td> 
        ...
        <div class="pagelist"> 
          <button type="submit" class="delete">删除所选</button> <div class="pagenum"><?php echo ($show); ?></div> 
        </div> 
      </form>
    </div>
  </div>
...
<script type="text/javascript">
  ...
  if(adm_power.indexOf('文章列表') === -1){
    ...
    $('.order').blur(function(){
      ...
      $.post('/Admin/Article/order',{'oldOrder':oldOrder,'order':order,'id':id},function(data){
        // console.log(data);
        if(!data){
          return false;
        }  
    ...
    $('.aa4').click(function(){
      ...
      var id = $(this).attr('artID');
      $.post('/Admin/Article/index',{'show':show,'id':id},function(data){
