...
<div class="rightfix">
  <a class="home1"></a><a class="home_pass">班级管理</a><a class="home_now">班级修改</a>
  <div class="clear"></div>
 	<div class="article_edit_list" style="width:1086px ;">
 		<form action="<?php echo U('/index.php/Admin/Product/editor',array('aid'=>$aid));?>" method='post' enctype="multipart/form-data">
    <div class="article_edit_list">
      <!-- 产品名称 -->    
      <div class="edit_title"><div class="col_left f">产品名</div><div class="col_right f"><input type="text" name="title" class="title_text" value="<?php echo ($pro[0]['pro_name']); ?>"/></div><div class="clear"></div></div> <div class="clear"></div>
      <!-- 产品分类 -->
      <div class="choose_sort same_height"> <div class="col_left f">选择分类</div> <div class="col_right f"> <select class="sort f" name="cate" id="cate"> <option value="<?php echo ($acate['cate_id']); ?>" ><?php echo ($acate['cate_name']); ?></option> 
      	<?php if(is_array($cate)): foreach($cate as $key=>$value): ?><option value="<?php echo ($value["cate_id"]); ?>" 
      		<?php if($pid == 0){if($pro['pro_cate_id'] == $value['cate_id']){echo "selected=selected" ;}}else{if($pid==$value['cate_id']){echo "selected=selected" ;}} ?>><?php echo ($value["cate_name"]); ?>
      		</option><?php endforeach; endif; ?> </select> <select class="sort f" name="scate" id="scate" style="margin-left:25px;"> <option value="" <?php if($pro['pro_cate_id'] != $val['cate_id']){echo "selected=selected";} ?>>未分类</option> <?php if(is_array($scate)): foreach($scate as $key=>$val): ?><option value="<?php echo ($val["cate_id"]); ?>" <?php if($pro['pro_cate_id'] == $val['cate_id']){echo "selected=selected";} ?>><?php echo ($val["cate_name"]); ?></option><?php endforeach; endif; ?> </select> <div class="f"><input type="text" class="newsort" style="width:120px;"><label class="newsorttext">新建分类</label></div> <div class="clear"></div> </div> </div> 
      		<div class="clear"></div>
      <!-- 产品缩略图 -->
      <div class="thumbnail"> <div class="col_left f">缩略图</div> <div class="col_right f"> <div class="img_containner f"> <img id="imghead" src='<?php echo ($pro[0]['pro_thumb']); ?>'> <img src='/Public/Admin/images/thumbnail_close.png' class='thumbnail_close'> </div> <div class="add_img_c"> <div class="fileimg"><input type="file" onchange="previewImage(this)" name="ban_url"/></div> </div> </div> <div class="clear"></div> </div> <div class="clear"></div>
      <!-- 产品是否显示 -->
      
      <div class="digest same_height"><div class="col_left f">摘要</div><div class="col_right f"><input type="text" name="summary" value="<?php echo ($pro[0]['pro_summary']); ?>" class="title_text Summary" /></div> </div><div class="clear"></div>
      <!-- 产品详细介绍 -->
      <div class="article_detail"><div class="col_left f d_bottom">产品介绍</div><div class="col_right f d_bottom"><div class="detail_edit"> <?php echo $htmlData; ?> <textarea name="content1" class="textarea">
      	<?php echo ($pro[0]['pro_introduction']); ?>
      </textarea> </div></div></div> <div class="clear"></div>
      <!-- 提交或者取消 -->
