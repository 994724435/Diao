...
<style>
	...
	.keep input{
		...
		margin-left: 10px;
		margin-bottom: 39px;
		background: url(/Public/Admin/images/delete.png);
	}
	.addimg{
		margin-left: 668px;
		padding-top: 29px;
...
<div class="rightfix">
  	<a class="home1"></a><a class="home_pass">友情链接</a><a class="home_now">图片链接</a><img src="/Public/Admin/images/addimg.jpg" alt="" onclick="window.location.href='/Admin/Link/img_add'" class="addimg"/>
  	<div class="clear"></div>
  	<div class="menberlist">
  		<div class='check_page'>
  			<span id="check_title">选择页面</span>
  			<select name="check_page" id="check_">	
  				<?php if(is_array($img_page)): foreach($img_page as $key=>$val): ?><option value="<?php echo ($val["bp_id"]); ?>" <?php if($bp_id == $val['bp_id']){echo "selected='selected'";} ?>><?php echo ($val["bp_name"]); ?></option><?php endforeach; endif; ?>
  			</select>
  		</div>
		<div class="menber_con">
			...
			<div class="menber_title">
  			...
  			<div class="clear"></div>
  			<?php if(is_array($img_link)): foreach($img_link as $key=>$val): ?><div class="photo_row">
  				<div class="photo_td1 f"><input type="checkbox" name="idarr[]" id="" value="<?php echo ($val["img_id"]); ?>"></div>
  				<div class="photo_td2 f"><img src="<?php echo ($val["img_url"]); ?>" alt="" style="width:375px;height:94px;"></div>
  				<div class="photo_td3 f"><input type="text" value="<?php echo ($val["img_name"]); ?>" imgid="<?php echo ($val["img_id"]); ?>"></div>
  				<div class="photo_td4 f"><input type="text" value="<?php echo ($val["img_order"]); ?>" imgid="<?php echo ($val["img_id"]); ?>"></div>
  				<div class="photo_td5 f"><input type="text" value="<?php echo ($val["img_gourl"]); ?>" imgid="<?php echo ($val["img_id"]); ?>"></div>
  				<div class="photo_td6 f"><a href="<?php echo U('/Admin/Link/img_del',array('img_id'=>$val['img_id']));?>">删除</a></div>
  			</div>
...
if(adm_power.indexOf('banner管理') === -1){
	...
	$('.keep input').addClass('friendShip1_notice_block show_block').attr('type','button');
}else{
...
$('#check_').change(function(){ 
	var bp_id = $(this).val();
	var url = '/Admin/Link/img_link/bp/'+bp_id;
