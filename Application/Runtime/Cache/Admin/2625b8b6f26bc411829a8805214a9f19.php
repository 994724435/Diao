  	...
  	<div class="menberlist">
		...
		<div class="menber_con">
			...
			<div class="menber_title">
	  			<div class="photo_col1 f menber_col">
	  				<input type="checkbox" class="allchoose"/>
	  				<label class="label_choose">全选</label>
	  			</div>
	  			<div class="photo_col2 f menber_col">
	  				预览图
	  			</div>
	  			<div class="photo_col3 f menber_col">
	  				名称
	  			</div>
	  			<div class="photo_col4 f menber_col">
	  				顺序
	  			</div>
	  			<!--<div class="photo_col5 f menber_col">
	  				链接地址
	  			</div>-->
	  			<div class="photo_col6 f menber_col">
	  				操作
	  			</div>
  			</div>
  			<div class="clear"></div>
  			<?php if(is_array($data)): foreach($data as $key=>$val): ?><div class="photo_row">
  				<div class="photo_td1 f"><input type="checkbox" name="idarr[]" value="<?php echo ($val["ban_id"]); ?>" id=""></div>
  				<div class="photo_td2 f"><img src="<?php echo ($val["ban_url"]); ?>" alt="" style="width:375px;height:94px;"></div>
  				<div class="photo_td3 f"><input type="text" value="<?php echo ($val["ban_name"]); ?>" banid="<?php echo ($val["ban_id"]); ?>"></div>
  				<div class="photo_td4 f"><input type="text" value="<?php echo ($val["ban_order"]); ?>" banid="<?php echo ($val["ban_id"]); ?>"></div>
  				<div class="photo_td6 f"><a href="<?php echo U('/Admin/Photo/bandel',array('ban_id'=>$val['ban_id']));?>">删除</a></div>
  			</div>
...
if(adm_power.indexOf('banner管理') === -1){
	$('.photo_td3 input,.photo_td4 input,.photo_td5 input').addClass('friendShip1_notice_block show_block');
	$('.photo_td6 a').addClass('friendShip1_notice_block show_block').attr('href','javascript:;');
}else{
	$('.photo_td3 input').blur(function(){
		var name = $(this).val();
		var id = $(this).attr('banid');
		$.post('/Admin/Photo/changeName',{'name':name,'id':id},function(data){
			if(data){
				return true;
			}
		});
	});
	$('.photo_td4 input').blur(function(){
		var order = $(this).val();
		var id = $(this).attr('banid');
		$.post('/Admin/Photo/changeOrder',{'order':order,'id':id},function(data){
			if(data){
				return true;
			}
		});
	});
	$('.photo_td5 input').blur(function(){
		var gourl = $(this).val();
		var id = $(this).attr('banid');
		$.post('/Admin/Photo/changeGourl',{'gourl':gourl,'id':id},function(data){
			if(data){
				return true;
			}
		});
...
$('#check_').change(function(){
	var bp_id = $(this).val();
	var url = '/Admin/Photo/index/bp/'+bp_id;
