...
<!--文章编辑-->
<div class="rightfix">
  <a class="home1"></a><a class="home_pass" href="/index.php/Admin/Product/index">班级管理</a><a class="home_now">添加班级</a>
  <div class="clear"></div>
  <div style="width:1072px;margin-left:5px;">
    <form action="<?php echo U('/index.php/Admin/Product/add');?>" method='post' enctype="multipart/form-data">
    <div class="article_edit_list">
      <div class="edit_title"><div class="col_left f">名称</div><div class="col_right f"><input type="text" name="title" class="title_text" value=""/></div><div class="clear"></div></div>
      <div class="clear"></div>
      <div class="choose_sort same_height">
        <div class="col_left f">选择分类</div>
        <div class="col_right f">
          <select class="sort f" name="cate" id="cate">
            <option value="" selected="selected">未分类</option>
            <?php if(is_array($cate)): foreach($cate as $key=>$value): ?><option value="<?php echo ($value["cate_id"]); ?>"><?php echo ($value["cate_name"]); ?></option><?php endforeach; endif; ?>
          </select>
      ...
      <div class="clear"></div>
        <div class="thumbnail">
          ...
          <div class="col_right f">
            ...
            <div class="add_img_c">
              <div class="fileimg"><input type="file" onchange="previewImage(this)" name="ban_url"/></div>
            </div>
          </div>
          <div class="clear"></div>
        </div>
        <div class="clear"></div>
        <div class="digest same_height"><div class="col_left f">摘要</div><div class="col_right f"><input type="text" name="summary" value="" class="title_text Summary" /></div> </div>
        <div class="clear"></div>
        <div class="article_detail"><div class="col_left f d_bottom">文章详情</div><div class="col_right f d_bottom"><div class="detail_edit">
        <?php echo $htmlData; ?>
        <textarea name="content1" class="textarea"><?php echo htmlspecialchars($htmlData); ?></textarea>
        </div></div></div>
        <div class="clear"></div>
        <div class="cancle_save"><input type="submit" class="article_submit button_big" value=""/> <input type="button" class="article_cancel button_big" value=""/></div>
        <div class="clear"></div>
      </div>
     </form>
     <div class="clear"></div>
...
if(adm_power.indexOf('添加产品') === -1){
  ...
  $("#cate").change(function(){
    var pid=$(this).val();
    if(pid){
      $.post('/index.php/Admin/Product/cateAjax',{'pid':pid},function(data){
        $('#scate').empty().append('<option value="">未分类</option>');
        for(i in data){
          $('#scate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>");
        }
      },'json');
    }else{
      $('#scate').empty().append('<option value="">未分类</option>');
  ...
  $(".newsorttext").click(function(){
    ...
    var cid = $("#cate").val();
    $.post('/index.php/Admin/Product/sortAjax',{'sort':sort,'cid':cid},function(data){
        if(!cid){
          $('#cate').empty().append('<option value="">未分类</option>');
          $('#scate').empty().append('<option value="">未分类</option>');
          for(i in data){
            $('#cate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>");
          }
        }else{
          $('#scate').empty().append('<option value="">未分类</option>');
          for(i in data){
