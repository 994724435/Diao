...
<div class="rightfix">
  ...
  <div style="width:1072px;margin-left:5px;">
    <form action="<?php echo U('/index.php/Admin/Member/add');?>" method='post'  enctype="multipart/form-data">
      <div class="article_edit_list">
        <!-- 标题 -->
        <div class="edit_title"><div class="col_left f">名 称</div><div class="col_right f"><input type="text" name="title" class="title_text" value=""/></div><div class="clear"></div></div> <div class="clear"></div>
        <!-- 文章分类 -->
        <div class="choose_sort same_height"> <div class="col_left f">选择分类</div> <div class="col_right f"> 
        	<select class="sort f" name="cate" id="cate"> <option value="" selected="selected">未分类</option> 
        		<?php if(is_array($data)): foreach($data as $key=>$value): ?><option value="<?php echo ($value["m_id"]); ?>"><?php echo ($value["m_name"]); ?></option><?php endforeach; endif; ?> 
