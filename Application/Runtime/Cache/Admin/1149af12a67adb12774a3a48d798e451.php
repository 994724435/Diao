...
<style> #preview{width:289px;height:93px;border:1px solid #cfcfcf;overflow:hidden;text-align: center;} #imghead {filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod=image);width:114px;height:104px;margin-top: 0;} .fileimg input{width: 100%; height: 55px; cursor: pointer; opacity: 0; filter: alpha(opacity=0); } </style>
 <div class="rightfix">
 <!-- 头信息 -->
  <a class="home1"></a><a class="home_pass">文章管理</a><a href="/Admin/Article/index" class="home_pass">文章列表</a><a class="home_now">文章编辑</a>
  <div class="clear"></div>

  <!-- 文章修改 -->
  <div style="width:1072px;margin-left:5px;">
    <form action="<?php echo U('/Admin/Article/editor',array('aid'=>$aid));?>" method='post' enctype="multipart/form-data">
      <div class="article_edit_list">
        <div class="edit_title"> <div class="col_left f">标题</div> <div class="col_right f"> <input type="text" name="title" class="title_text" value="<?php echo ($art["art_title"]); ?>"/> </div> <div class="clear"></div> </div> <div class="clear"></div>
   		   <!-- 选择文章分类 -->
        <div class="choose_sort same_height"> <div class="col_left f">选择分类</div> <div class="col_right f"> <select class="sort f" name="cate" id="cate"> <option value="" >未分类</option> <?php if(is_array($cate)): foreach($cate as $key=>$value): ?><option value="<?php echo ($value["cate_id"]); ?>" <?php if($pid == 0){if($art['art_cateid'] == $value['cate_id']){echo "selected=selected" ;}}else{if($pid==$value['cate_id']){echo "selected=selected" ;}} ?>><?php echo ($value["cate_name"]); ?></option><?php endforeach; endif; ?> </select> <select class="sort f" name="scate" id="scate" style="margin-left:25px;"> <option value="" <?php if($art['art_cateid'] != $val['cate_id']){echo "selected=selected";} ?>>未分类</option> <?php if(is_array($scate)): foreach($scate as $key=>$val): ?><option value="<?php echo ($val["cate_id"]); ?>" <?php if($art['art_cateid'] == $val['cate_id']){echo "selected=selected";} ?>><?php echo ($val["cate_name"]); ?></option><?php endforeach; endif; ?> </select> <div class="f"> <input type="text" class="newsort" style="width:120px;"> <label class="newsorttext">新建分类</label> </div> <div class="clear"></div> </div> </div> <div class="clear"></div>
  	 		<!-- 上传缩略图 -->
        <div class="thumbnail"> <div class="col_left f">缩略图</div> <div class="col_right f"> <div class="img_containner f"> <img id="imghead" src='<?php echo ($art["art_thumb"]); ?>'> </div> <div class="add_img_c"> <div class="fileimg"><input type="file" onchange="previewImage(this)" name="thumb"/></div> </div> </div> <div class="clear"></div> </div> <div class="clear"></div>
  	 		<!-- 上传作者 -->
        <div class="submit_person same_height"> <div class="col_left f">提交人</div> <div class="col_right f"> <input type="text" name="author" class="title_text" value="<?php echo ($art["art_author"]); ?>" /> </div> <div class="clear"></div> </div> <div class="clear"></div>
  	 		<!-- 文章发布日期 -->
        <div class="public same_height"> <div class="col_left f">发表日期</div> <div class="col_right f"> <input type="text" name="time" class="title_text" readonly="true" value="<?php echo (date("Y-m-d",$art["art_addtime"])); ?>" /> </div> <div class="clear"></div> </div> <div class="clear"></div>
  	 		<!-- 文章来源 -->
        <div class="source same_height"> <div class="col_left f">来源</div> <div class="col_right f"> <input type="text" class="title_text" name="source" value="<?php echo ($art["art_from"]); ?>" /> </div> <div class="clear"></div> </div> <div class="clear"></div>
  	 		<!-- 文章访问权限 -->
        <div class="permissions same_height"> <div class="col_left f">访问权限</div> <div class="col_right f"> <input type="radio" name="isshow" value="1" <?php if($art['art_isshow']==1){echo "checked='checked'";} ?> class="article_radios"/> <label>所有用户访问</label> <label></label> <input type="radio" name="isshow" value="0" <?php if($art['art_isshow']==0){echo "checked='checked'";} ?> class="article_radios1"/> <label>仅会员访问</label> </div> <div class="clear"></div> </div> <div class="clear"></div>
  	 		<!-- 文章摘要 -->
        <div class="digest same_height"> <div class="col_left f">摘要</div> <div class="col_right f"> <input type="text" name="summary" value="<?php echo ($art["art_summary"]); ?>" class="title_text Summary" /> </div> </div> <div class="clear"></div>
  	 		<!-- 文章内容 -->
        <div class="article_detail"> <div class="col_left f d_bottom">文章详情</div> <div class="col_right f d_bottom"> <div class="detail_edit"> <?php echo $htmlData; ?> <textarea name="content1" class="textarea"><?php echo ($art["art_content"]); echo htmlspecialchars($htmlData); ?></textarea> </div> </div> </div> <div class="clear"></div>
  	 		<!-- 提交或取消 -->
...
$("#cate").change(function(){
  var pid=$(this).val(); if(pid){$.post('/Admin/Article/cateAjax',{'pid':pid},function(data){$('#scate').empty().append('<option value="">未分类</option>'); for(i in data){$('#scate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>"); } },'json'); }else{$('#scate').empty().append('<option value="">未分类</option>'); } 
});

$(".newsorttext").click(function(){
  var sort = $(".newsort").val(); var cid = $("#cate").val(); $.post('/Admin/Article/sortAjax',{'sort':sort,'cid':cid},function(data){if(!cid){$('#cate').empty().append('<option value="">未分类</option>'); $('#scate').empty().append('<option value="">未分类</option>'); for(i in data){$('#cate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>"); } }else{$('#scate').empty().append('<option value="">未分类</option>'); for(i in data){$('#scate').append("<option value='"+data[i]['cate_id']+"'>"+data[i]['cate_name']+"</option>"); } } },'json');
});
