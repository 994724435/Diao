...
//		print_r($result);die;
    ...
    public function delete(){
        ...
        if($date){
            echo "<script>alert('删除成功');window.location.href='/index.php/Admin/Product/index';</script>";exit;
        }else{
            echo "<script>alert('删除失败');window.location.href='/index.php/Admin/Product/index';</script>";exit;
        }
    }
    //产品批量删除
    public function formDelete(){       
        ...
        if($aid){           //如果$aid有值
            ...
            if($data){ 
                echo "<script>window.location.href='/index.php/Admin/Product/index';</script>";exit;
            }else{
                echo "<script>alert('删除失败');window.history.go(-1)";exit;
            }
            
        }else{
            echo "<script>window.location.href='/index.php/Admin/Product/index';</script>";exit;      //如果$aid没值，直接跳转到产品首页
        }
    }
    ...
    public function editor(){
        ...
        $pro=$product->where(array('pro_id'=>$aid))->select();
//      print_r($pro);die;
        $acate = $procate->where(array('cate_id'=>$pro[0]['pro_cate_id']))->find();
        $pid = $acate['cate_pid'];
        if($pid == 0){
...
//      	print_r($_POST);die;
			...
			$id =I('post.id'); 	
            $res=$product->where(array('pro_id'=>$id))->save($arr);
            if($_FILES['ban_url']){
            	    $id =I('post.id'); 	
                    $thumb = imgFile();
                    $info = $thumb['info'];
                    if(!$info) {// 上传错误提示错误信息
                        ...
                        $src=$p.$img;
		                $product = M('product');	
                         $product->where(array('pro_id'=>$id))->save(array('pro_thumb'=>$src));
                        echo "<script>alert('修改成功');window.location.href='/Admin/Product/index';</script>";
                    }
                }else{
...
//		print_r($data);die;
    ...
    public function fir_class(){    //一级分类添加
        if(IS_POST){
            ...
            $res = $cate->add($data);
            if($res){
                echo "<script>window.location.href='/index.php/Admin/Product/sort';</script>";
            }
        }
    }
    ...
    public function cateDelete(){
        ...
        if($cid){
            ...
            if($data){
                    echo "<script>window.location.href='/index.php/Admin/Product/sort';</script>";exit;
                }else{
                    echo "<script>alert('删除失败');window.location.href='/index.php/Admin/Product/sort';</script>";exit;
                }
